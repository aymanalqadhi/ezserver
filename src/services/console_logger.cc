...
std::ostream& ezserver::services::ConsoleLogger::Log(const LoggingLevel &level)
{
    ...
    auto timestamp = IncludeTime() ? "[" + ezserver::utils::time::GetTimeString(time(0), "%H:%M:%S") + "]" : "";

    switch(level)
    {
        // Trace level: used to log exception trace calls and messages
        case LoggingLevel::kTrace:
            return writing_stream_ << termcolor::dark << timestamp << "[Trace] " << termcolor::reset;

        // Debug level: used for debugging purposes
        case LoggingLevel::kDebug:
            return writing_stream_ << termcolor::grey << termcolor::bold << timestamp << "[Debug] " << termcolor::reset;

        // Information level: used to log usual messages
        case LoggingLevel::kInformation:
            return writing_stream_ << termcolor::cyan << termcolor::bold << timestamp << "[Info]  " << termcolor::reset;

        // Warning level: used to log warning messages
        case LoggingLevel::kWarning:
            return writing_stream_ << termcolor::yellow << termcolor::bold << timestamp << "[Warn]  " << termcolor::reset;

        // Error level: used to log recoverable errors messages
        case LoggingLevel::kError:
            return writing_stream_ << termcolor::red << timestamp << "[Error] " << termcolor::reset;

        // Fatal level: used to log unrecvoerable erros messaegs
        case LoggingLevel::kFatal:
            return writing_stream_ << termcolor::red << termcolor::bold << timestamp << "[Fatal] " << termcolor::reset;
    }
